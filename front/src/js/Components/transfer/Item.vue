<template>
    <table class="table table-striped table-bordered">
        <tr  v-if="item.id"><td>{{ __('common.id') }}</td><td>{{ item.id }}</td></tr>
        ...
        <tr>
            ...
            <td>
                <input type="datetime-local" id="start-time" 
                   disabled
                    class="input" :value="item.start_time"   />
        ...
        <tr>
            ...
            <td>
                <input type="datetime-local" id="end-time" 
                    disabled
                    class="input" :value="item.end_time"   />
        ...
        getSeat:{
            get:function(){
                ...
                for (let i = 1; i <= this.item.passenger_capacity; i++) {
                    ...
                    output.push({'type': this.multiple ? 'checkbox' : 'radio','disabled': found ,'val':i}) ;                     
                }
                return output;
            }
        },
